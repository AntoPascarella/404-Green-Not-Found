...
folders <- list(
  FP16 = file.path(output_dir, "falcon-3b-fp16_logs"),
  INT8 = file.path(output_dir, "falcon-3b-int8_logs"),
  INT4 = file.path(output_dir, "falcon-3b-int4_logs")
)
...
if(length(removed_logs)>0){
  ...
  write.csv(removed_df_all[,c("model","file_name","run_id","reason")],
            file=file.path(output_dir,"falcon_removed_outliers.csv"), row.names=FALSE)
...
write.csv(final_summary,
          file=file.path(output_dir, "falcon_quantization_extended_summary.csv"),
          row.names=FALSE)
...
p_box <- ggplot(plot_data, aes(x=model, y=value, fill=model)) +
  ...
  theme_bw() +
  labs(title="Boxplots of Metrics by Quantization Level (Falcon)") +
  scale_fill_manual(values=c("FP16"="blue","INT8"="orange","INT4"="green"))
ggsave(filename=file.path(output_dir,"falcon_all_metrics_boxplot.png"), plot=p_box,
...
p_dens <- ggplot(plot_data, aes(x=value, fill=model)) +
  ...
  facet_wrap(~metric, scales="free", ncol=2) +
  theme_bw() +
  labs(title="Density Plots of Metrics by Quantization Level (Falcon)") +
  scale_fill_manual(values=c("FP16"="blue","INT8"="orange","INT4"="green"))
ggsave(filename=file.path(output_dir,"falcon_all_metrics_density.png"), plot=p_dens,
       dpi=300, width=12, height=8)

# === Statistical tests with transformation and automatic parametric/non-parametric selection ===
...
for(m in metrics){
  ...
  sw_results <- df_metric %>% group_by(model) %>%
    summarise(shapiro_p = shapiro.test(value_trans)$p.value, .groups="drop")
  
  # --- Apply transformation if any group is non-normal ---
  if(any(sw_results$shapiro_p <= 0.05)){
  ...
  if(all(sw_results_post$shapiro_p > 0.05)){
...
# Save results
write.csv(stat_results_all, file=file.path(output_dir,"falcon_statistical_tests_final.csv"), row.names=FALSE)
